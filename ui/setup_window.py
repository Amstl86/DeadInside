...
class SetupWindow(ctk.CTkToplevel):
    def __init__(self, on_goal_added):
        ...
        self.goal_label = ctk.CTkLabel(
            self, text="Что ты хочешь достичь? 🎯", font=(
                "Segoe UI", 16, "bold"))
        self.goal_label.pack(pady=(30, 10))

        self.goal_entry = ctk.CTkEntry(
            self,
            placeholder_text="Например: выучить испанский до B1",
            width=400,
            height=40)
        ...
        self.radio_days = ctk.CTkRadioButton(
            self.radio_frame,
            text="Указать количество дней",
            variable=self.method_var,
            value="days",
            command=self.toggle_method)
        self.radio_days.grid(row=0, column=0, padx=10, pady=5, sticky="w")

        self.radio_calendar = ctk.CTkRadioButton(
            self.radio_frame,
        ...
        self.days_label = ctk.CTkLabel(
            self.days_frame, text="Количество дней до дедлайна:")
        self.days_label.pack(side="left", padx=5)
        self.days_entry = ctk.CTkEntry(
            self.days_frame, width=60, justify="center")
    ...
    def set_goal_action(self):
        ...
        if self.method_var.get() == "days":
            ...
            if days < 1:
                messagebox.showerror(
                    "Ошибка", "Число дней должно быть больше 0")
                return
            ...
            if deadline <= date.today():
                messagebox.showerror(
                    "Ошибка", "Дата дедлайна должна быть в будущем!")
                return
        ...
        if success:
            ...
            messagebox.showerror(
                "Ошибка",
                "Не удалось добавить цель. Возможно, превышен лимит.")
