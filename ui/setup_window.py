...
class SetupWindow(ctk.CTkToplevel):
    """
    Окно для установки новой цели при первом запуске приложения.

    Это модальное окно предлагает пользователю ввести текст цели и выбрать дату дедлайна
    двумя способами: через календарь или указанием количества дней от текущей даты.
    После сохранения данных вызывает переданный callback для перехода к основному интерфейсу.

    Atributs:
        on_goal_set (callable): функция, вызываемая после успешного сохранения цели
            (не принимает аргументов, запускает главное окно приложения).

    Methods:
        __init__(on_goal_set_callback): конструктор, создаёт все виджеты и настраивает окно.
        set_goal_action(): собирает данные из полей ввода, валидирует их, сохраняет через
            data_manager и вызывает callback.
        show_error(msg): отображает всплывающее сообщение об ошибке.

    Примечания:
        - Дата создания цели фиксируется автоматически через set_creation_date().
        - Приоритет отдаётся дате из календаря; если она не установлена, используется
          количество дней из второго поля.
        - Окно не изменяет размер, следует системной теме оформления (светлой/тёмной).
    """

    def __init__(self, on_goal_set):
        super().__init__()
        ...
        # Выбор способа
        self.method_var = ctk.StringVar(value="days")  # по умолчанию используем дни
        ...
        # Панель для календаря (изначально скрыта)
        ...
        # По умолчанию календарь скрыт, т.к. active radio = days
    ...
    def set_goal_action(self):
        ...
        # Определяем дедлайн в зависимости от выбранного метода
        if self.method_var.get() == "days":
            ...
            if deadline <= date.today():
                self.show_error("Дата дедлайна должна быть в будущем!")
                return
    ...
    def show_error(self, msg):
        # Простой вывод ошибки через messagebox (нужен импорт из tkinter)
