...
from utils.autostart import is_in_startup
import pystray
...
class MainWindow(ctk.CTk):
    ...
    def refresh_goals_list(self):
        ...
        title = ctk.CTkLabel(
            self.list_frame, text="Мои цели", font=(
                "Segoe UI", 20, "bold"))
        title.pack(pady=20)

        if not goals:
            empty = ctk.CTkLabel(self.list_frame,
    ...
    def create_goal_card(self, goal):
        ...
        goal_label = ctk.CTkLabel(
            info_frame, text=goal["goal"], font=(
                "Segoe UI", 14, "bold"))
        ...
        days_text = (
            f"Осталось дней: {days_left}" if days_left is not None
            else ""
        )
        if days_text:
            days_lbl = ctk.CTkLabel(
                info_frame, text=days_text, font=(
                    "Segoe UI", 11))
    ...
    def create_tray_icon(self):
        ...
        draw.ellipse([4, 4, 60, 60], fill='#2e7d32',
                     outline='#1b5e20', width=3)
