...
class GoalDetailView(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        days_text = (
            f"Осталось дней: {days_left}" if days_left > 0
            else "🔥 Дедлайн сегодня! 🎉"
        )
        self.days_label = ctk.CTkLabel(
            self, text=days_text, font=(
                "Segoe UI", 24, "bold"))
        self.days_label.pack(pady=5)

        # Поле заметки
        note_label = ctk.CTkLabel(
            self, text="Что ты сделал сегодня? 📝", font=(
                "Segoe UI", 14))
        ...
        history_label = ctk.CTkLabel(
            self, text="📋 История заметок", font=(
                "Segoe UI", 16, "bold"))
        history_label.pack(pady=(15, 5))

        self.history_frame = ctk.CTkScrollableFrame(
            self, width=500, height=200)
    ...
    def populate_history(self):
        ...
        if not notes:
            empty_label = ctk.CTkLabel(
                self.history_frame, text="Пока нет заметок.")
